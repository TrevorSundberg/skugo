import "./modal.css";
import $ from "jquery";
import {Deferred} from "../../shared/utility";
...
export class Modal {
  ...
  public async open (params: ModalOpenParameters): Promise<ModalButton> {
    ...
    if (params.content) {
      if (typeof params.content === "string") {
        body.append($("<p/>").text(params.content));
      } else {
  ...
  public static async messageBox (title: string, contents: string | JQuery, dismissable = true): Promise<ModalButton> {
